...
tau_square_gt = 1.
true_value_pars = np.array((tau_square_gt))
...
Omega = thin_plate_spline_kernel_xpart(x[:,np.newaxis])
...
Beta_process = sp.multivariate_normal.rvs(mean = np.zeros(N), cov=tau_square_gt*np.eye(N))
...
while len(Beta_process_samples) < n_iterations:
    ...
    tau_square_p = sp.invgamma.rvs(a = num_eigen/2., scale = b_inverse_gamma + np.dot(Beta_process_c, np.transpose(Beta_process_c))/2.)
    ...
    prior_beta_process_p = sp.multivariate_normal.pdf(Beta_process_p, np.zeros(num_eigen), np.eye(num_eigen)*tau_square_p)
    prior_beta_process_c = sp.multivariate_normal.pdf(Beta_process_c, np.zeros(num_eigen), np.eye(num_eigen)*tau_square_c)
    q_beta_process_c = sp.multivariate_normal.pdf(Beta_process_c, beta_hat_c, tau_square_c*cov_beta_hat_c)
    q_beta_process_p = sp.multivariate_normal.pdf(Beta_process_p, beta_hat_p, tau_square_p*cov_beta_hat_p)
    ...
    pred_mean, pred_covariance = predictiveGP_spline(np.dot(X_subset, Beta_process_c), tau_square_c, Omega, N, x, x_star)

    ### Store object of the MCMC
    tau_square_samples.append(tau_square_c)
...
tau_square_mcmc = np.mean(np.array(tau_square_samples[burn_in:]))
...
samples_vector = [np.array(tau_square_samples[burn_in:]), np.array(Beta_process_samples)[burn_in:, :]]
